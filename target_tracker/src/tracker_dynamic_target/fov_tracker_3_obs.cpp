  ...
  ArrayXXf tot_time_up)
{
  ...
  prob_data.x_obs = (hstack(hstack(x_temp_1, x_temp_2), x_temp_3)).transpose();
  prob_data.y_obs = (hstack(hstack(y_temp_1, y_temp_2), y_temp_3)).transpose();

  prob_data.a_obs = ArrayXXf(3, 1);
  prob_data.b_obs = ArrayXXf(3, 1);
  prob_data.c_obs = ArrayXXf(3, 1);
  prob_data.a_obs << 1.3, 1.3, 1.3;
  prob_data.b_obs << 2.2, 1.2, 1.2;

  // prob_data.a_obs << 0.8, 0.8, 0.8;
  // prob_data.b_obs << 0.8, 0.8, 0.8;
