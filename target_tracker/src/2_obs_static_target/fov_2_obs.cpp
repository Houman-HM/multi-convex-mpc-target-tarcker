...
Eigen::ArrayXXf optim::compute_A_occ(Eigen::ArrayXXf P, ProbData prob_data, Eigen::ArrayXXf u)
{
  ...
  temp_mat_1 = (1 - u(0)) * P;
  A_occ = vstack(temp_mat_1, temp_mat_1);
  ...
  float rho_obs, float rho_occ, float rho_fov_z, ArrayXXf u)
{
    ...
    ArrayXXf x_occ = vstack(x_occ_1, x_occ_2);
    ArrayXXf y_occ = vstack(y_occ_1, y_occ_2);
    ...
    ArrayXXf  b_ineq_x = prob_data.B_ineq_x - prob_data.s_ineq_x - prob_data.lamda_ineq_x / rho_ineq;
    ArrayXXf  b_ineq_y = prob_data.B_ineq_y - prob_data.s_ineq_y - prob_data.lamda_ineq_y / rho_ineq;
...
ProbData optim::wrapper_fov(ProbData prob_data, Eigen::ArrayXXf P, Eigen::ArrayXXf Pdot, Eigen::ArrayXXf Pddot, Eigen::ArrayXXf u, float delt_factor)
{
  ...
  for (size_t i = 0; i < prob_data.maxiter; i++)
  {
    ...
    ArrayXXf x_tilda = vstack(x_tilda_temp, x_tilda_temp);
    ArrayXXf y_tilda = vstack(y_tilda_temp, y_tilda_temp);
  ...
  ArrayXXf P, ArrayXXf Pdot, ArrayXXf Pddot, ArrayXXf tot_time, ArrayXXf u)
{
  ...
  prob_data.weight_smoothness = 50.0;
  ...
  prob_data.x_obs = (hstack(hstack(x_temp_1, x_temp_2), prob_data.r_1(0) * ArrayXXf::Ones(prob_data.num,1))).transpose();
  prob_data.y_obs = (hstack(hstack(y_temp_1, y_temp_2), prob_data.r_1(1) * ArrayXXf::Ones(prob_data.num,1))).transpose();
  prob_data.z_obs = (hstack(1.0 * ArrayXXf::Ones(prob_data.num,1), prob_data.r_1(2) * ArrayXXf::Ones(prob_data.num,1))).transpose();

  prob_data.a_obs = ArrayXXf(3, 1);
  prob_data.b_obs = ArrayXXf(3, 1);
  prob_data.c_obs = ArrayXXf(3, 1);
  prob_data.a_obs << 0.8, 0.8, 1.0;
  prob_data.b_obs << 0.8, 0.8, 1.0;
  prob_data.c_obs << 3.0, 3.0, 1.0;

  prob_data.A_obs = P.replicate(prob_data.num_obs,1);
  

  prob_data.a_occ = (vstack(prob_data.a_obs(0) * ArrayXXf::Ones(prob_data.num_samples,1), prob_data.a_obs(1) * ArrayXXf::Ones(prob_data.num_samples,1)));
  prob_data.b_occ = (vstack(prob_data.b_obs(0) * ArrayXXf::Ones(prob_data.num_samples,1), prob_data.b_obs(0) * ArrayXXf::Ones(prob_data.num_samples,1)));
