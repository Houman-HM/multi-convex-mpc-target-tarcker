...
ros::Publisher drone_cmd_pub, obs_1_pub, obs_2_pub, obs_3_pub;
float obs_1_vx, obs_1_vy, obs_2_vx, obs_2_vy, obs_3_vx, obs_3_vy, obs_velocity_publisher_delay;
              ...
              const nav_msgs::Odometry::ConstPtr& obs_2_odom, const nav_msgs::Odometry::ConstPtr& obs_3_odom)
{
  ...
  obs_3_pose << obs_3_odom->pose.pose.position.x, obs_3_odom->pose.pose.position.y, obs_3_odom->pose.pose.position.z;;
}
...
void MPCThread()
{
  ...
  YAML::Node config = YAML::LoadFile(package_path + "/params/3_obs_params.yaml");
  ProbData prob_data;
  ProbData sim_data;
  ...
  sim_data.x_obs_3_init = config["Params"]["sim_data.x_obs_3_init"].as<float>();
  sim_data.y_obs_3_init = config["Params"]["sim_data.y_obs_3_init"].as<float>();
  ...
  sim_data.obs_3_vx = config["Params"]["sim_data.obs_3_vx"].as<float>();
  sim_data.obs_3_vy = config["Params"]["sim_data.obs_3_vy"].as<float>();
  ...
  ArrayXXf x_obs_3_pos = sim_data.x_obs_3_init * ArrayXXf::Ones(1, prob_data.mpc_iter);
  ArrayXXf y_obs_3_pos = sim_data.y_obs_3_init * ArrayXXf::Ones(1, prob_data.mpc_iter);
  ...
  for (size_t i = 1; i <prob_data.mpc_iter; i++)
  {
    ...
    x_obs_1_pos(i) = obs_1_pose(0);
    y_obs_1_pos(i) = obs_1_pose(1);
    x_obs_2_pos(i) = obs_2_pose(0);
    y_obs_2_pos(i) = obs_2_pose(1);

    x_obs_3_pos(i) = obs_3_pose(0);
    y_obs_3_pos(i) = obs_3_pose(1);
    ...
    sim_data.y_obs_3_init = y_obs_3_pos(i);
    sim_data.x_obs_3_init = x_obs_3_pos(i);
...
void obstacleVelPublisherThread()
{
  ...
  while(ros::ok())
  {
    ...
    obs_3_vel.linear.x = obs_3_vx;
    obs_3_vel.linear.y = obs_3_vy;
...
message_filters::Subscriber<nav_msgs::Odometry> obs_3_odom_sub(n, "/obs_3/odom", 1);
typedef message_filters::sync_policies::ApproximateTime<nav_msgs::Odometry, nav_msgs::Odometry, nav_msgs::Odometry, nav_msgs::Odometry> MySyncPolicy;
message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(100), drone_odom_sub, obs_1_odom_sub, obs_2_odom_sub, obs_3_odom_sub);
